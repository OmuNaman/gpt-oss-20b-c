...
static inline float mxfp4_dot_row(const uint8_t* blocks, const uint8_t* scales,
...
#if USE_SIMD
    // vpshufb MXFP4: parallel 4-bit LUT lookup replaces scalar nibble extraction.
    // Int8 LUT encodes FP4 values * 2 (range -12..+12 fits int8).
    // The 2x factor is cancelled by using (exponent - 128) instead of (exponent - 127).
    static const int8_t fp4_i8_data[16] __attribute__((aligned(16))) = {
        0, 1, 2, 3, 4, 6, 8, 12, 0, -1, -2, -3, -4, -6, -8, -12
    };
    const __m128i fp4_lut = _mm_load_si128((const __m128i*)fp4_i8_data);
    ...
    for (int g = 0; g < n_groups; g++) {
        ...
        // Load all 16 packed bytes = 32 FP4 values at once
        __m128i raw = _mm_loadu_si128((const __m128i*)gb);

        // Extract low nibbles (even indices) and high nibbles (odd indices)
        __m128i lo_nibs = _mm_and_si128(raw, nib_mask);
        __m128i hi_nibs = _mm_and_si128(_mm_srli_epi16(raw, 4), nib_mask);

        // Parallel LUT lookup: 16 byte-table lookups each in 1 cycle (vpshufb)
        __m128i lo_i8 = _mm_shuffle_epi8(fp4_lut, lo_nibs);
        __m128i hi_i8 = _mm_shuffle_epi8(fp4_lut, hi_nibs);

        // Interleave to match x[] order: [lo[0],hi[0], lo[1],hi[1], ...]
